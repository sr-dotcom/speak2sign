...
| App smoke | `streamlit.testing.v1.AppTest` | 5 | local + CI | The page runs and shows the disclaimer and a curated ribbon; the typed lane renders and its result survives a rerun; a forecast failure and an engine failure render as messages, not tracebacks |
| Review tooling | pytest with a stub `codex` | 5 |
