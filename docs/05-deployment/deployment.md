...
| 2026-09-10 | c17a36c | CI green (memory gate now loads T5 from the release: peak 386 MB on the runner). Live URL checked three times over 15 minutes after the push: the host was still serving the previous build (identified by a note string that changed in this commit), so this deploy is **UNVERIFIED**. Owner action: open the app at share.streamlit.io, check the deploy log for c17a36c, and reboot the app if the redeploy did not trigger. |
